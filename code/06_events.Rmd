...
```{r events_no_data, echo = FALSE, fig.align='center', warning = FALSE, message = FALSE}
...
```{r events_minimal_data, echo = FALSE, fig.align='center',warning = FALSE, message = FALSE}
...
```{r event_trends, echo = FALSE, warning = FALSE, fig.align='center',message = FALSE, fig.width = 9, fig.height = 5}
end_date <- Sys.Date() - 1
...
if(evt_ttl != 0 & evt_ttl != evt_no_data_ttl){
  ...
  get_event_daily_trend <- function(event_id, prefix = ""){
    ...
    gg <- ggplot(df, aes(x = day, y = value, group = 1)) + 
      ...
      geom_hline(yintercept = mean(df$value), color = "blue", linetype='dotted', lwd = .8, alpha = 0.5) +
      ...
      labs(title = paste0(prefix, event_id, ": ", df$name[[1]]),
           subtitle = paste0('Daily average of ',event_id, ' was: ', round(mean(df$value), 0)),
           caption = paste("Source: Adobe Analytics. Report Suite:", rsid, "-", format(start_date, "%d %b"), "to", format(end_date, "%b %d"))) +
      theme_bw() +
