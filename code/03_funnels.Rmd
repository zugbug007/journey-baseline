...
Period: From: **`r format(first_valid_date, "%d %b")`** To: **`r format(last_valid_date, "%d %b")`**
...
Previous Week: **`r format(fourteen_days_ago, "%d-%b")` to `r format(fourteen_days_ago+6, "%d-%b")`**.  
Last 7 Days: **`r format(seven_days_ago, "%d-%b")` to `r format(seven_days_ago+6, "%d-%b")`**.
...
```{r membership_traffic_forecast, echo=FALSE, fig.height=5, fig.width=7, fig.align='center', message=FALSE, warning=FALSE}
journey_name_forecast <- "Commercial: Membership Checkout Steps 1-4"
forecast_metric <- "Membership Revenue (ev5)"
get_forecast(journey_name_forecast, forecast_metric)

forecast_metric <- "Membership Step 4.0 - Confirmation (Serialized) (ev26)"
get_forecast(journey_name_forecast, forecast_metric)
...
**Week on Week performance**
**All traffic through the Renewal Funnel, see note below.**  

**NOTE** Due to Renewals from emails linking directly into step 2 of the funnel, this may cause some over inflation of step 2 as renewals are sent out to customers in the first or second week of the month.
...
#renew_pre_post_email_funnel
```

**Week on Week performance**
**EMAIL ONLY traffic through the Renewal Funnel, see note below.**  

**NOTE** Due to Renewals from emails linking directly into step 2 of the funnel, this funnel isolates only those renewals that have come via the email renewal message. These visitors skip step one and enter directly into step 2 due to the pre-authentication email link.
...
```{r renew_funnel_pre_post_email, fig.align='center',echo=FALSE}
...
```{r renew_traffic_forecast, echo=FALSE, fig.height=5, fig.width=7, fig.align='center',message=FALSE, warning=FALSE}
journey_name_forecast <- "Commercial: Renew Checkout Steps 1-3"

forecast_metric <- "Renew Revenue - Serialized (ev79)"
get_forecast(journey_name_forecast, forecast_metric)

forecast_metric <- "Renew Step 3.0 - Confirmation - Serialized (ev76)"
get_forecast(journey_name_forecast, forecast_metric)
...
```{r holidays_traffic_forecast, echo=FALSE, fig.height=5, fig.width=7, fig.align='center', message=FALSE, warning=FALSE}
journey_name_forecast <- "Commercial: Holidays Checkout Steps 1-4"

forecast_metric <- "Holidays Booking Total Revenue (Serialised) (ev125)"
get_forecast(journey_name_forecast, forecast_metric)

forecast_metric <- "Holidays Booking Step 4.0 - Confirmation - Serialised (ev134)"
get_forecast(journey_name_forecast, forecast_metric)
...
**NOTE:** Due to Touring Pass sales requiring no delivery address, these sales skip over step 2 (delivery details), and move directly to step 3.
This may cause some under counting in step 2 and over inflation of step 3.
...
**NOTE** Due to Touring Pass sales requiring no delivery address, these sale skip over step 2 and move directly to step 3.
This may cause some under counting in step 2 and over inflation of step 3.
...
```{r shop_traffic_forecast, echo=FALSE, fig.height=5, fig.width=7, fig.align='center',message=FALSE, warning=FALSE}
journey_name_forecast <- "Commercial: Shop Checkout Steps 1-4"

forecast_metric <- "Shop - Revenue"
get_forecast(journey_name_forecast, forecast_metric)

forecast_metric <- "Shop - Order Step 4 - Order Confirmation (Serialized) (ev182)"
get_forecast(journey_name_forecast, forecast_metric)
...
```{r donate_traffic_forecast, echo=FALSE, fig.height=5, fig.width=7, fig.align='center',message=FALSE, warning=FALSE}
journey_name_forecast <- "Commercial: Donate Checkout Steps 1-2"

forecast_metric <- "Donate Revenue (Serialized) (ev114)"
get_forecast(journey_name_forecast, forecast_metric)

forecast_metric <- "Donate Step 2.0 - Complete (Serialized) (ev116)"
get_forecast(journey_name_forecast, forecast_metric)
